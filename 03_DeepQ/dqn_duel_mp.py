...
if __name__ == '__main__':
    ...
    net = model.DuelDQNet(params.OBS_SIZE, params.ACT_SIZE)
    net.share_memory()
    print(net)
    ...
    for n in range(params.N_MP):
        proc = mp.Process(target=mp_utils.data_fun, name=str(
            n), args=(net, exp_queue, params))
        proc.start()
    ...
    for batch in generator:
        ...
        loss = utils.calc_dqn_loss(
            batch, net, tgt_net, params.GAMMA**params.STEPS)
        loss.backward()
        optimizer.step()
        epoch += 1
