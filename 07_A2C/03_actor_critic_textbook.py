...
class A2CNet(nn.Module):
    def __init__(self, obs_size, act_size):
        super().__init__()
        self.base = nn.Sequential(nn.Linear(obs_size, 128),
                                  nn.ReLU())
        self.policy = nn.Linear(128, act_size)
...
if __name__ == '__main__':
    ...
    for exp in exp_source:
        ...
        if new_reward:
            ...
            if delta_time > 2:
                print(
                    f'Epoch:{epoch:3}, Loss: {loss_v:3.2f}, Reward:{new_reward[0]:6.2f}, Mean Rewards:{mean:6.2f}')
                print_time = time()
    ...
    if args.save:
        torch.save(net.state_dict(), 'trainedModels/actorCritic.dat')
    if args.play:
        play(envs[0], agent)
